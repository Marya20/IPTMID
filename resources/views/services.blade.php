<section class="page-section" id="services"><br><br>
            <div class="container">
                <div class="text-center">
                    <h3 class="section-subheading text-muted">These companies do great things for our pets. Be sure to check them out!</h3>
                </div><br>
                <div class="row text-center">
                    <div class="col-md-4">
                        <span class="fa-stack fa-4x">
                        <i class="fa fa-circle fa-stack-2x"></i>
                            <i class="fa fa-money fa-stack-1x fa-inverse"></i>
                        </span>
                        <h4 class="my-3">Donate</h4>
                    </div>
                    <div class="col-md-4">
                        <span class="fa-stack fa-4x">
                        <i class="fa fa-circle fa-stack-2x"></i>
                            <i class="fa fa-heart fa-stack-1x fa-inverse"></i>
                        </span>
                        <h4 class="my-3">Adopt</h4>
                    </div>
                    <div class="col-md-4">
                        <span class="fa-stack fa-4x">
                        <i class="fa fa-circle fa-stack-2x"></i>
                            <i class="fa fa-users fa-stack-1x fa-inverse"></i>
                        </span>
                        <h4 class="my-3">Volunter</h4>
                    </div>
                </div>
            </div>
        </section><br><br><br><br>
        <!-- Team-->
 <section class="page-section bg-light" id="team"><br><br><br>
            <div class="container">
                <div class="text-center">
                    <h2 class="section-heading text-uppercase">Our Amazing Team</h2>
                    <h4 class="section-subheading text-muted">Lorem ipsum dolor sit amet consectetur.</h4>
                </div>
                <div class="row">
                    <div class="col-lg-3">
                        <div class="team-member">
                            <img class="mx-auto rounded-circle" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQG2jem7XycBdP1tb2a7z8LxmcRlx_FIJ1piA&usqp=CAU" alt="..." />
                            <h4>Mary Bagsoling</h4>
                            <p class="text-muted">Veterenarian</p>
                        </div>
                    </div>
                    <div class="col-lg-3">
                        <div class="team-member">
                        <img class="mx-auto rounded-circle" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ6w0iLn47qlyZKqAWdYuahcG16QzNFa0dt4YieTcNzyzvtVJ6NuVlzHGEa8Or1qa02cwY&usqp=CAU" alt="..." />
                            <h4>Joanna Smith</h4>
                            <p class="text-muted">Veterinary Assistant</p>
                        </div>
                    </div>
                    <div class="col-lg-3">
                        <div class="team-member">

                            <img class="mx-auto rounded-circle" src="data:image/png;base64," alt="..." />
                            <h4>Glenn Parker</h4>
                            <p class="text-muted">Veterinary Technician</p>
                        </div>
                    </div>
                    <div class="col-lg-3">
                        <div class="team-member">
                        <img class="mx-auto rounded-circle" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQNC2hazYO0ZLNkB_aIGtZdlgINq0q-AneWOg&usqp=CAU" alt="..." />
                            <h4>Larry Larry</h4>
                            <p class="text-muted">Rescue Leader</p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-8 mx-auto text-center"><p class="large text-muted">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aut eaque, laboriosam veritatis, quos non quis ad perspiciatis, totam corporis ea, alias ut unde.</p></div>
                </div>
            </div>
        </section>
    
        <section class="page-section bg-light" id="portfolio"><br><br><br><br><br><br>
            <div class="container">
                <div class="text-center">
                    <h6 class="section-heading text-uppercase text-danger">IMPORTANT INFORMATION REGARDING BUSINESS HOURS:</h6>
                    <p class="section-subheading text-muted">PAWS Shelter will be holding adoptions by appointment only. We are not accepting walk-in foot traffic at this time.
Please click here to view our most recent changes to business hours due to the COVID-19 pandemic.</p>
                </div>
                <br><br> 
        </section>